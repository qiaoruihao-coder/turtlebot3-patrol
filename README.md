...
> - `--watch 2.545 0.514` = 等机器人接近投放点 (2.545, 0.514) 时**自动投放**障碍物（演示主力）。`ros2 run` 启动需 15~40s 才就绪，必须**紧跟终端 A 尽快运行**，否则机器人先过了投放点就不投放了。
...
**步骤 2（终端 B，紧跟步骤 1 尽快运行）** —— 动态避障投放：
```bash
# 智能监控: 等机器人接近投放点时自动投放（演示主力, 推荐）
# 投放点 (2.545, 0.514) = 去 wp3 的必经缝隙(东), 机器人会自动绕西缝隙(较远路线)
ros2 run tb3_patrol spawn_obstacle --watch 2.545 0.514
```
> ⚠️ `ros2 run` 启动需 15~40s 才就绪，必须**紧跟步骤 1 尽快运行**，否则机器人先过了投放点就不投放了。
